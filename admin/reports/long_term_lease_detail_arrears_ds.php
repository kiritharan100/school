<?php
// long_term_lease_detail_arrears_ds.php
// DS-wise Long Term Lease Arrears Summary (matches full report logic)
...
/* ---------------------------------------------------------
   3. LOAD ALL ACTIVE LEASES WITH lease_type FILTER
--------------------------------------------------------- */
$sql = "
SELECT 
    l.lease_id,
...
WHERE 
    ...
    AND l.start_date <= ?
    AND (?='All' OR l.type_of_project=?)
";
$stmt = mysqli_prepare($con, $sql);
mysqli_stmt_bind_param($stmt, "sss", $asAt, $leaseType, $leaseType);
...
/* ---------------------------------------------------------
   5. PROCESS ALL LEASES (USE FULL REPORT LOGIC)
--------------------------------------------------------- */
foreach($leases as $L){
    ...
    if (!isset($groups[$cid])) continue; // skip invalid DS

    $groups[$cid]['leases']++;
    $groups[$cid]['extent'] += (float)$L['extent_ha'];

    /* ----------------------------------------------------
       5A. LOAD SCHEDULES (full report logic)
    ---------------------------------------------------- */
    $sch = [];
    $q = mysqli_query($con,"
    ...
    /* ----------------------------------------------------
       5B. LOAD PAYMENTS (full report logic)
    ---------------------------------------------------- */
    $pay = [];
    $q = mysqli_query($con,"
    ...
    /* ----------------------------------------------------
       5D. PAYMENT SPLIT LOGIC (FOLLOW ORIGINAL SCRIPT)
    ---------------------------------------------------- */
    $rentPaidOpenPrev=0;
    $rentPaidOpenCurr=0;
    $discOpenPrev=0;
    ...
    /* ----------------------------------------------------
       5E. FINAL CALCULATIONS (MATCH ORIGINAL REPORT)
    ---------------------------------------------------- */
    ...
    $currentYearRentOutstanding = 
        $currentYearRentDue - $rentPaidCurrYr - $discCurrYr;

    /* Penalty for year (same as original) */
    $cyPenaltyDue=0;
    foreach($sch as $S){
        $sd=$S['start_date'];
        if ($sd >= $currYearStart && $sd <= $asAt){
    ...
    /* ----------------------------------------------------
       5F. ADD TO DS GROUP TOTAL
    ---------------------------------------------------- */
    $groups[$cid]['opening_rent_arrears'] += $openingRentArrears;
    $groups[$cid]['arrears_paid_cy']      += $rentPaidOpenCurr;
    $groups[$cid]['opening_rent_balance'] += $balanceOpeningRent;

    $groups[$cid]['penalty_opening']      += $openingPenaltyDue;
    $groups[$cid]['penalty_paid_opening'] += $penaltyPaidOpening;
    $groups[$cid]['penalty_balance_opening'] += $openingPenaltyBal;

    $groups[$cid]['opening_outstanding']  += $openingOutstanding;

    $groups[$cid]['cy_rent_due']          += $currentYearRentDue;
    $groups[$cid]['cy_discount']          += $discCurrYr;
    $groups[$cid]['cy_rent_received']     += $rentPaidCurrYr;
    $groups[$cid]['cy_outstanding']       += $currentYearRentOutstanding;
    $groups[$cid]['cy_penalty']           += $currentYearPenaltyBalance;

    $groups[$cid]['year_end_outstanding'] += $yearEndOutstanding;

    $groups[$cid]['premium_opening_bal']  += $openingPremiumBalance;
    $groups[$cid]['premium_opening_paid'] += $openingPremiumPaid;

    $groups[$cid]['outstanding_to_date']  += $outstandingToDate;

    $groups[$cid]['total_payments_year']  += $totalPaymentForYear;
}
...
<head>
<meta charset="utf-8">
<title>DS-wise Lease Arrears Summary</title>
<style>
...
<h3>DS-wise Long Term Lease Arrears Summary</h3>
As at: <b><?=h($asAt)?></b><br>
